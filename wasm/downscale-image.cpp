...
ProcessImageResult downscaleImageUtil(const Image& img, int grid, 
    ...
    debug_log("Input image dimensions: " + std::to_string(width) + "x" + std::to_string(height));
    debug_log("Input pixel count: " + std::to_string(img.pixels.size()));

    // Calculate grid dimensions
    int gridWidth = grid;
    int gridHeight = static_cast<int>(std::round(static_cast<double>(height) * gridWidth / width));
    double cellWidth = static_cast<double>(width) / gridWidth;
    double cellHeight = static_cast<double>(height) / gridHeight;

    debug_log("Output grid dimensions: " + std::to_string(gridWidth) + "x" + std::to_string(gridHeight));
    debug_log("Cell dimensions: " + std::to_string(cellWidth) + "x" + std::to_string(cellHeight));

    // Initialize color quantizer if needed
    ...
    if (colorQuantization && quantizationType == "kmeans") {
        ...
        debug_log("Found " + std::to_string(dominantColors.size()) + " dominant colors");
    }
    ...
    for (int y = 0; y < gridHeight; y++) {
        ...
        for (int x = 0; x < gridWidth; x++) {
            ...
            if (rs.empty()) {
                debug_log("Warning: Empty cell at " + std::to_string(x) + "," + std::to_string(y));
                continue;
            }
            ...
            Color medianColor = {
                static_cast<uint8_t>(rs[medianIndex]),
            ...
            if (colorQuantization && !dominantColors.empty() && quantizationType == "kmeans") {
                medianColor = kmeans->findClosestColor(medianColor, dominantColors);
            }
    ...
    debug_log("Output image dimensions: " + std::to_string(output.width) + "x" + std::to_string(output.height));
...
ProcessImageResult processImage(const std::string& base64Image, int gridSize,
    ...
    // Process the image
    return downscaleImageUtil(img, gridSize, colorTolerance, 
                             colorQuantization, maxColors, quantizationType);
}
...
Image decodeBase64Image(const std::string& base64Data) {
    ...
    debug_log("Decoded base64 data size: " + std::to_string(decodedData.size()));
    ...
    if (error) {
        debug_log("LodePNG decode error: " + std::string(lodepng_error_text(error)));
        throw std::runtime_error("Failed to decode image: " + std::string(lodepng_error_text(error)));
    }

    debug_log("Decoded image dimensions: " + std::to_string(width) + "x" + std::to_string(height));
...
std::string encodeImageToBase64(const Image& image) {
    ...
    debug_log("Encoding image dimensions: " + std::to_string(image.width) + "x" + std::to_string(image.height));
    ...
    if (error) {
        debug_log("LodePNG encode error: " + std::string(lodepng_error_text(error)));
